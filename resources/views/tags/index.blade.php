...
@section('content')
    <div class="max-w-screen-xl mx-auto pt-28 pb-52 p-5">
        <div>
            <h1 class="sans text-xl text-extrabold animate__animated animate__fadeInDown">Manage Tag</h1>
        </div>
                    ...
                    @forelse ($data as $item)
                        <tr
                            ...
                            <td class="px-6 py-4 flex ">
                                <a href="{{ route('tags.edit', Crypt::encrypt($item->id)) }}"
                                    class="font-medium text-green-600 mr-2">
                                    <i class="fas fa-pencil"></i>
                                </a>
                                <form onsubmit="return confirm('Are you sure delete this data ?');"
                                    action="{{ route('tags.destroy', Crypt::encrypt($item->id)) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
