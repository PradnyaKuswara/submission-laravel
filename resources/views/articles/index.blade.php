...
@section('content')
    <div class="max-w-screen-xl mx-auto pt-28 pb-52 p-5">
        <div>
            <h1 class="sans text-xl text-extrabold animate__animated animate__fadeInDown">Manage Articles</h1>
        </div>
        ...
        <div class="mt-4 relative overflow-x-auto shadow-lg sm:rounded-lg animate__animated animate__fadeIn">
            ...
            <table class="w-full text-sm overflow-y text-left text-gray-500 dark:text-gray-400">
                <thead class="border-b text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 border-r w-60">
                            Content
                        </th>
                        <th scope="col" class="px-6 py-3 border-r w-60">
                            Category
                ...
                <tbody>
                    @forelse ($data as $item)
                        <tr
                            ...
                            <td class="px-6 py-4 border-r truncate block w-60">
                                {{ $item->content }}
                            </td>
                            <td class="px-6 py-4 border-r">
                            ...
                            <td class="px-6 py-4 flex ">
                                <a href="{{ route('articles.edit', Crypt::encrypt($item->id)) }}"
                                    class="font-medium text-green-600 mr-2">
                                    <i class="fas fa-pencil"></i>
                                </a>
                                <form onsubmit="return confirm('Are you sure delete this data ?');"
                                    action="{{ route('articles.destroy', Crypt::encrypt($item->id)) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
